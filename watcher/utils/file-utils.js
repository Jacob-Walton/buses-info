...
function verifyEntryPoints(throwOnMissing = false) {
	...
	if (!fs.existsSync(config.sourceDir)) {
		const error = new Error(
			`Source directory "${config.sourceDir}" not found!`,
		);
	...
	const allValid =
		results.js.missing.length === 0 && results.scss.missing.length === 0;
...
function printValidationSummary(validationResult) {
	...
	console.log(
		`JavaScript files: ${results.js.found}/${results.js.found + results.js.missing.length} found`,
	);
	console.log(
		`SCSS files: ${results.scss.found}/${results.scss.found + results.scss.missing.length} found`,
	);
