class SettingsManager {
	...
	async loadPreferenceSettings() {
		...
		if (data) {
			...
			const showPreferredRoutesFirst = this.getElement(
				"showPreferredRoutesFirst",
			);
			if (showPreferredRoutesFirst) {
				showPreferredRoutesFirst.checked =
					data.showPreferredRoutesFirst || false;
			}
		}
	}
	...
	async loadNotificationSettings() {
		const data = (await this.cache.preferences) || (await this.loadAllData());
		if (data) {
			const enableEmailNotifications = this.getElement(
				"enableEmailNotifications",
			);
			if (enableEmailNotifications) {
				enableEmailNotifications.checked =
					data.enableEmailNotifications || false;
			}
		}
	}
	...
	setupEventListeners() {
		...
		document
			.querySelectorAll('[name="PreferredRoutes"]')
			.forEach((checkbox) => {
	...
	checkFormChanges() {
		...
		const hasChanges =
			JSON.stringify(currentState) !== JSON.stringify(this.initialState);
	...
	populateRoutes(container, selectedRoutes) {
		...
		const dropdownTrigger = container
			.closest(".custom-dropdown")
			?.querySelector(".selected-options span");
		if (dropdownTrigger) {
