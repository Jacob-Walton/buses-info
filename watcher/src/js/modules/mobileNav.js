export function initializeMobileNav() {
	...
	function initTouchHandling() {
		...
		isTouchDevice = true;
		let navbarWidth = elements.navbar.offsetWidth || 300;
		...
		elements.navbar.addEventListener('touchend', (e) => {
			if (!elements.navbar.classList.contains('active')) return;
		...
		document.addEventListener('touchend', (e) => {
			// Only handle this when menu is closed
			if (elements.navbar.classList.contains('active') || touchStartX === 0) return;
	...
	// Handle links click
	document.querySelectorAll(".mobile-nav-link, .mobile-profile-item a, .mobile-profile-item button").forEach((link) => {
	...
	window.addEventListener("resize", () => {
		if (window.innerWidth > 768 && elements.navbar.classList.contains("active")) {
			closeMobileMenu();
		}
	});
