...
pub async fn export_user_data(
    ...
    // Get user basic info
    let user_info = get_user_info(&database, &user_id).await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    
    // Get user preferences (if any exist)
    let preferences = get_user_preferences(&database, &user_id).await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    
    // Get usage data
    let usage_data = get_usage_data(&database, &user_id).await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn delete_user_account(
    ...
    match result {
        Ok(_) => Ok(Json(AccountDeletionResponse {
            ...
            message: "Your account and all associated data have been permanently deleted.".to_string(),
        })),
...
async fn get_user_info(database: &Database, user_id: &str) -> Result<UserInfo, Box<dyn std::error::Error>> {
...
async fn get_user_preferences(database: &Database, user_id: &str) -> Result<UserPreferences, Box<dyn std::error::Error>> {
...
async fn get_usage_data(database: &Database, user_id: &str) -> Result<UsageData, Box<dyn std::error::Error>> {
...
async fn delete_user_data(database: &Database, user_id: &str) -> Result<(), Box<dyn std::error::Error>> {
