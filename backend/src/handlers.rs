...
pub async fn health_status(
    ...
    let redis_connection = if let Some(redis_service) = &redis {
        redis_service.health_check().await.is_ok()
    } else {
        false
    };
...
pub async fn current_bus_information(
    State((_db, cache, _redis)): State<(Database, BusCache, Option<RedisService>)>,
) -> impl IntoResponse {
